...
BPF.attach_raw_socket(function_http_filter, "ens2f2")

ni.ifaddresses('ens2f2')
ip = ''
...
print("MachineIP   ipver     Src IP Addr     src Port     Dst IP Addr    Dst Port  Packet Length   protocol  Local_Src_Addr  Local_des_Addr  VNI VLANID  ")
count_c1 = 0
...
#  print("ipver     Src IP Addr     src Port     Dst IP Addr    Dst Port  Packet Length   protocol   timestamp")
  ...
  #socket.gethostname(),
  MESSAGE = (ip, str(int(ipversion, 2)), srcAddr, srcPort, dstAddr, dstPort, str(total_length), protocol, local_src_addr, local_des_addr, str(int(VNI)), str(int(VLANID)))
